...
const api = {
  ...
  /** Kicks off a cloud scrape run (Vercel bridge -> GitHub Actions).
   * @param {string} targetUrl
   * @returns {Promise<{ jobId?: string, error?: string }>} */
  cloudTriggerScrape: (targetUrl) => ipcRenderer.invoke('cloud-trigger-scrape', targetUrl),

  /** Polls Turso for a cloud job's status/leads so far.
   * @param {string} jobId
   * @returns {Promise<{ status?: string, leadCount?: number, leads?: object[], error?: string }>} */
  cloudPollJob: (jobId) => ipcRenderer.invoke('cloud-poll-job', jobId)
}
